...
(function($) {
    ...
    $(document).ready(function() {
        ...
        // Slet postet Facebook-opslag fra meta box
        $(document).on('click', '.fb-delete-fb-post', function(e) {
            e.preventDefault();
            
            var button   = $(this);
            var spinner  = button.siblings('.fb-delete-fb-post-spinner');
            var postId   = button.data('post-id');
            var postIndex = button.data('post-index');
            var fbPostId = button.data('fb-post-id');
            
            if (!confirm(fbPostScheduler.deleteFbPostConfirm || 'Er du sikker på, at du vil slette dette opslag fra Facebook? Handlingen kan ikke fortrydes.')) {
                return;
            }
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action:     'fb_post_scheduler_delete_fb_post',
                    nonce:      fbPostScheduler.nonce,
                    post_id:    postId,
                    post_index: postIndex,
                    fb_post_id: fbPostId
                ...
                success: function(response) {
                    spinner.removeClass('is-active');
                    if (response.success) {
                        button.closest('p.fb-post-success').find('.fb-delete-fb-post').remove();
                        button.closest('p.fb-post-success').find('.fb-delete-fb-post-spinner').remove();
                        showNotice(response.data.message, 'success');
                    } else {
                        button.prop('disabled', false);
        ...
        // Slet postet Facebook-opslag fra admin-listen
        $(document).on('click', '.fb-delete-fb-post-admin', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        button.prop('disabled', false).text('Slet fra Facebook');
                        var msg = response.data && response.data.message ? response.data.message : 'Der opstod en fejl';
                        showNotice(msg, 'error');
                    }
                },
